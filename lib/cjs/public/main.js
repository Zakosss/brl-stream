...
function formatTime(ms, noMs = false) {
    ...
    let pad = 3
    let msString = `.${ms.toString().padEnd(pad, '0')}`
    ...
    if (m == 0) {
        return `${s.toString().padStart(1, '0')}${msString}`
    } else {
        return `${m}:${s.toString().padStart(2, '0')}${msString}`
    }
}

const shortSafetyCarStatus = {
    "No Safety Car": "",
    "Full Safety Car": "SC",
...
const safetyCarStatusColours = {
    "No Safety Car": "rgb(0,194,28)",
    "Full Safety Car": "rgb(253, 224, 71)",
    "Virtual Safety Car": "rgb(253, 224, 71)",
    "Formation Lap": "rgb(255,255,255)"
}
...
class Leaderboard {
    ...
    constructor(element) {
        ...
        this.driversByPosition = new Proxy(this.drivers, {
            get(target, prop, reciever) {
                let chosenDriver

                target.forEach((driver, index) => {
                    if (driver.position == prop) {
                        chosenDriver = driver
                    }
                })
    ...
    #updateLapCounter() {
        if (this.ruleset == "Race") {
            this.#element.find('#counter').text(`${this.#lap}/${this.#totalLaps}`)
        } else {
            this.#element.find('#counter').text(formatTime(this.#remainingTime*1000, true))
        }
    }
    ...
    set session(sessionName) {
        this.#session = sessionName.toUpperCase()
        this.#element.find('#session').text(this.#session)
    }
    ...
    set safetyCarStatus(safetyCarStatus) {
        if (safetyCarStatus != this.#safetyCarStatus) {
            this.#safetyCarStatus = safetyCarStatus

            this.expand(this.#safetyCarStatus != "No Safety Car")

            this.#element.find('#safetyCarText').text(shortSafetyCarStatus[this.#safetyCarStatus])

            $('body').css("--flag1", safetyCarStatusColours[this.#safetyCarStatus])
            $('body').css("--flag2", safetyCarStatusColours[this.#safetyCarStatus])
        }
    }
}
...
class Driver {
    ...
    set pitStatus(pitStatus) {
        ...
        this.#pitStatus = statuses[pitStatus]
    }
    ...
    set interval(interval) {
        ...
        if (leaderboard.ruleset == "Race") {
            if (this.#pitStatus == "none") {
                if (this.#position == 1) {
                ...
                this.#element.find('#time').text(this.#pitStatus)
            }
        } else {
            if ((this.#times.bestTimes.total != Number.MAX_SAFE_INTEGER) && (this.#times.bestTimes.total != 0)) {
                this.#element.find('#time').text(`${formatTime(this.#times.bestTimes.total)}`)
            }
        }
    }
            ...
            */
                ...
                for (const [id, lapData] of Object.entries(data)) {
                    if (!(drivers[id] === undefined)) { // if index exists
                        ...
                        driver.pitStatus = lapData.pitStatus
                    }
                }
            ...
            } else if (event.name == 'sessionData') {
                leaderboard.totalLaps = data.totalLaps
                leaderboard.session = data.sessionType[longOrShort[data.ruleset]]
                leaderboard.ruleset = data.ruleset
                leaderboard.remainingTime = data.sessionTimeLeft
                leaderboard.safetyCarStatus = data.safetyCarStatus
            }
        }
    }
})